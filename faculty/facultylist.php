<?php
session_start();
require_once '../../config/database.php';

// Check if user is logged in and is faculty
if (!isset($_SESSION['user_id']) || $_SESSION['role'] !== 'faculty') {
    header('Location: ../login.php');
    exit();
}

$faculty_id = $_SESSION['user_id'];

// Get search parameter
$search = isset($_GET['search']) ? $_GET['search'] : '';

// Build faculty query
$query = "SELECT f.faculty_id, f.name, f.email, GROUP_CONCAT(DISTINCT c.course_name) as courses 
          FROM faculty f 
          LEFT JOIN courses c ON f.faculty_id = c.faculty_id 
          WHERE 1=1";

if ($search) {
    $query .= " AND (f.faculty_id LIKE ? OR f.name LIKE ? OR f.email LIKE ?)";
}

$query .= " GROUP BY f.faculty_id ORDER BY f.name";

$stmt = $conn->prepare($query);

if ($search) {
    $search_param = "%$search%";
    $stmt->bind_param("sss", $search_param, $search_param, $search_param);
}

$stmt->execute();
$result = $stmt->get_result();
$faculty_list = $result->fetch_all(MYSQLI_ASSOC);
?>

<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Faculty List - Solidarios</title>
    <link rel="stylesheet" href="../../assets/css/style.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css">
</head>
<body>
    <div class="container">
        <nav class="sidebar">
            <div class="logo">
                <h2>Solidarios</h2>
            </div>
            <ul class="nav-links">
                <li>
                    <a href="facultyindex.php">
                        <i class="fas fa-home"></i>
                        <span>Dashboard</span>
                    </a>
                </li>
                <li>
                    <a href="students.php">
                        <i class="fas fa-users"></i>
                        <span>Student List</span>
                    </a>
                </li>
                <li class="active">
                    <a href="faculty_list.php">
                        <i class="fas fa-chalkboard-teacher"></i>
                        <span>Faculty List</span>
                    </a>
                </li>
                <li>
                    <a href="courses.php">
                        <i class="fas fa-book"></i>
                        <span>My Courses</span>
                    </a>
                </li>
                <li>
                    <a href="grades.php">
                        <i class="fas fa-graduation-cap"></i>
                        <span>Grades</span>
                    </a>
                </li>
                <li>
                    <a href="../logout.php">
                        <i class="fas fa-sign-out-alt"></i>
                        <span>Logout</span>
                    </a>
                </li>
            </ul>
        </nav>

        <main class="main-content">
            <header>
                <div class="header-content">
                    <h1>Faculty List</h1>
                    <div class="user-info">
                        <span>Faculty ID: <?php echo htmlspecialchars($faculty_id); ?></span>
                    </div>
                </div>
            </header>

            <div class="content-section">
                <div class="filters">
                    <form method="GET" class="search-form">
                        <div class="search-box">
                            <input type="text" name="search" placeholder="Search by ID, Name, or Email" value="<?php echo htmlspecialchars($search); ?>">
                            <button type="submit"><i class="fas fa-search"></i></button>
                        </div>
                    </form>
                </div>

                <div class="table-container">
                    <table>
                        <thead>
                            <tr>
                                <th>Faculty ID</th>
                                <th>Name</th>
                                <th>Email</th>
                                <th>Courses</th>
                                <th>Actions</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php foreach ($faculty_list as $faculty): ?>
                                <tr>
                                    <td><?php echo htmlspecialchars($faculty['faculty_id']); ?></td>
                                    <td><?php echo htmlspecialchars($faculty['name']); ?></td>
                                    <td><?php echo htmlspecialchars($faculty['email']); ?></td>
                                    <td><?php echo htmlspecialchars($faculty['courses'] ?? 'No courses assigned'); ?></td>
                                    <td>
                                        <a href="view_faculty.php?id=<?php echo $faculty['faculty_id']; ?>" class="btn btn-view">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        <a href="mailto:<?php echo htmlspecialchars($faculty['email']); ?>" class="btn btn-email">
                                            <i class="fas fa-envelope"></i>
                                        </a>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
                </div>
            </div>
        </main>
    </div>

    <script src="../../assets/js/main.js"></script>
</body>
</html> 